    ...
    public function onClose(\swoole_server $server, int $fd, int $reactorId)
    {
        ...
        foreach ($table as $key=>$item) {
            ...
            if ($info && $info['websocket_status'] == WEBSOCKET_STATUS_FRAME) {
                ...
                $str = json_encode(array('act'=>'leave','code'=>0,'err'=>'' , 'msg'=> $res['name'].' 已离开'));
                $server->push($key, $str);

                $sendUser = [];
                foreach ($buff as $k => $vs) {
                    if ($k == $key ) {  //过滤自己
                        continue;
                    }
                    $sendUser[] = $vs;
                ...
                $str2 = json_encode(array('act'=>'getAllUser',  'code'=>0,  'err'=>'' , 'data'=>$sendUser));
                $server->push($key, $str2);
            }
